...
export default function WealthProjectionsPage() {
  ...
  const getCellClasses = (
    ...
    if (isMonth) {
      ...
      if (key === "year") {
        return `${base} pr-2 pl-4 text-[0.6875rem] text-white ${monthBg} ${align}`.trim();
      }
      if (key === "age") {
        return `${base} px-2 text-[0.6875rem] text-white ${monthBg} ${align}`.trim();
      }
      if (key === "deposit" || key === "revenue" || key === "endBalance") {
        return `${base} px-2 text-[0.6875rem] font-semibold text-white ${monthBg} ${align}`.trim();
      }
      return `${base} px-2 text-[0.6875rem] text-white ${monthBg} ${align}`.trim();
    }
    ...
    if (key === "year") {
      return `${base} pr-2 text-[0.6875rem] text-white ${hoverBg}`.trim();
    }
    if (key === "age") {
      return `${base} px-2 text-[0.6875rem] text-white ${hoverBg}`.trim();
    }
    if (key === "deposit" || key === "revenue" || key === "endBalance") {
      return `${base} px-2 text-[0.6875rem] font-semibold text-white ${hoverBg} ${align}`.trim();
    }
    return `${base} px-2 text-[0.6875rem] text-white ${hoverBg} ${align}`.trim();
  };
  ...
  return (
    <main className="mx-auto max-w-6xl w-full space-y-6 px-0 py-6 text-slate-100 lg:py-8">
      ...
      <section className="mt-4 grid gap-4 md:grid-cols-5">
        {fiveYearRates.map((r) => (
          ...
          >
            <div className="text-[0.6875rem] text-slate-400">{r.year}</div>
      ...
      <section className={`${surface} p-5 md:p-6`}>
        ...
        <div className="mt-4 overflow-x-auto">
          <table className="min-w-full text-left text-xs text-slate-200">
            <thead>
              <tr className="border-b border-slate-800 text-[0.6875rem] uppercase tracking-wide text-slate-500">
                  ...
                  >
                    <td className="py-2 pr-3 align-top">
                      <div className="flex flex-col">
                        ...
                        <span className="mt-0.5 text-[0.6875rem] text-slate-500">
                          Ends around {monthLabel(endMonth)}
                        </span>
                      </div>
                    </td>
                    <td className="px-3 py-2 align-top text-[0.6875rem] text-slate-400">
                      {inst.currency}
                    </td>
                    <td className="px-3 py-2 align-top text-right text-[0.6875rem] font-semibold text-white">
                      {formatCurrency(inst.principal, inst.currency)}
                    </td>
                    <td className="px-3 py-2 align-top text-right text-[0.6875rem] text-slate-300">
