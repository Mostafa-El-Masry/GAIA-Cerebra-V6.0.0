...
export default function ActivityFeed() {
  const [rows, setRows] = useState<
    { key: string; when: number; route?: string }[]
  >([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      for (const [k, raw] of Object.entries(snapshot)) {
        ...
        const when = new Date(
          v?.updatedAt || v?.createdAt || v?.date || 0,
        ).getTime();
        if (when > 0) {
          let route: string | undefined = undefined;
          if (k.startsWith("apollo_")) route = "/apollo";
          else if (k.startsWith("instagram_")) route = "/instagram";
          ...
          else if (k.startsWith("labs_")) route = "/apollo/learning";
