...
export type UseInstagramDataOptions = {
  /** When set, use this seed for shuffling instead of localStorage (e.g. "explore_seed" for Explore page). */
  fixedShuffleSeed?: string;
  /** When true, randomize order on each visit (new seed per mount). Use for main feed. */
  randomizeOrder?: boolean;
};

export function useInstagramData(options?: UseInstagramDataOptions) {
  ...
  const [shuffleSeedStorage] = useLocalStorage("gaia_gallery_shuffle_seed", () =>
    Math.random().toString(36).slice(2, 10),
  );
  const [randomSeed] = useState(() =>
    Math.random().toString(36).slice(2, 12),
  );
  const shuffleSeed = options?.fixedShuffleSeed
    ? options.fixedShuffleSeed
